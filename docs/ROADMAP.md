...
_Goal: a healthy, deployable skeleton with CI, so every later milestone lands on solid ground._
...
- **M0.1 — Monorepo scaffold.** pnpm workspaces; `apps/api`, `apps/mobile`,
  ...
  `.env.example`.
  _Accept:_ `pnpm install && pnpm typecheck && pnpm lint && pnpm build` all green
  on a clean checkout.
- **M0.2 — CI.** GitHub Actions: install → typecheck → lint → format:check →
  ...
  / `pnpm audit` step.
  _Accept:_ a trivial PR shows the full green check suite; `main` can't be pushed
  to directly.
- **M0.3 — API skeleton + DB.** Fastify server, env‑config module (validated),
  ...
  fresh DB).
  _Accept:_ `docker compose up` + `pnpm dev:api` → `/health` 200; migrations
...
- **M0.4 — Auth.** Better Auth: email/password sign‑up (with verification email
  ...
  _Accept:_ integration tests cover sign‑up → verify → login → authed `me` →
...
- **M0.5 — Shared package v1.** Domain types & Zod schemas for the entities we'll
  ...
  _Accept:_ API imports schemas from `packages/shared`; type‑level contract test
...
- **M0.6 — Client skeletons.** `apps/mobile`: Expo app boots (Expo Router),
  ...
  _Accept:_ both clients run locally, sign up / log in / log out against the API,
...
clients, deployable image. _(No checkpoint — nothing to "feel" yet.)_

---

## Phase 1 — MVP: the dog + Scout (the core loop) 🔒🧪

_Goal: do intake for a dog, open chat, and Scout knows the dog + breed and coaches you — safely._
...
- **M1.1 — Dog profile + intake.** `dog` schema/migration/CRUD; the full intake
  ...
  _Accept:_ create/edit a dog via UI on mobile + web; intake answers persist
...
- **M1.2 — Breed library v1.** `breed_profile` schema/migration + seed: a curated
  ...
  (or "mix of …").
  _Accept:_ search a breed, see its profile; the default dog shows the composite
...
- **M1.3 — Claude integration (server) 🔒.** Anthropic SDK wired in `apps/api`;
  **OAuth via `claude setup-token`** (env `ANTHROPIC_AUTH_TOKEN`) with
  `ANTHROPIC_API_KEY` fallback _(you generate the token — see WORKING_AGREEMENT)_;
  ...
  _Accept:_ a curl/integration test streams a real Scout reply that correctly
...
- **M1.4 — Scout chat UI.** Chat screen on mobile + web: streaming text, tool‑use
  ...
  _Accept:_ full back‑and‑forth chat works on both clients with live streaming;
...
- **M1.5 — Onboarding glue.** First‑run flow: sign up → intake → "meet Scout"
  ...
  _Accept:_ a brand‑new user goes from nothing to chatting with a
  Scout‑who‑knows‑their‑dog in one smooth flow on both clients.

**🧪 Checkpoint 1 — "Does talking to Scout feel good and _informed_?"** I deploy
to staging; you run intake (try the default _and_ a custom breed/age), chat with
...
_Goal: intake → a real, breed‑ and age‑aware, multi‑month/year training program you can follow and log against._
...
- **M2.1 — Program data model.** `program` / `program_phase` / `program_module` /
  ...
  **track** (Obedience · Socialization · Advanced · Protection) and a state
  ...
  _Accept:_ a program tree can be created, edited, reordered, track‑filtered, and
...
- **M2.2 — Skill & trick catalog v1.** `skill` schema/migration + seed from the
  ...
  _Accept:_ browse the catalog; open a skill and see its full shaping plan;
...
- **M2.3 — Curriculum generation.** `generate_program(dogId)` /
  ...
  _Accept:_ the default Mal × Dutch intake produces a coherent, sensibly
...
- **M2.4 — Program UI.** The **timeline‑of‑command‑cards** screen from
  ...
  on mobile + web.
  _Accept:_ the default dog's generated program renders as the artifact‑style
...
- **M2.5 — Training session logging.** `training_session` schema/migration; log a
  ...
  _Accept:_ log sessions three ways; charts render; "how's our recall going?"
...
_Goal: a complete health timeline, scheduled care, working reminders, and safe health coaching._
...
- **M3.1 — Health records.** `health_event` (polymorphic) + `media` schema/
  ...
  _Accept:_ build a full health history via UI; charts render; Scout's
...
- **M3.2 — Medications & preventives.** `medication` schema/migration; add a med
  ...
  _Accept:_ add meds & preventives, log doses, see adherence; Scout can list
...
- **M3.3 — Care schedules.** Rule‑driven schedules: puppy vaccine series, annual/
  ...
  _Accept:_ the default puppy shows a realistic puppy‑series + first‑year
...
- **M3.4 — Reminders engine ⚙️🔒.** `reminder` schema/migration; the in‑process
  ...
  Resend) _(you provide push credentials & an email sender — see
  WORKING_AGREEMENT)_; snooze/complete; `create_reminder` / `list_due_reminders`
  / `complete_reminder` tools; notification‑prefs settings made real.
  _Accept:_ a due/overdue reminder appears in‑app, fires a push on a real device,
...
- **M3.5 — Health UI + Scout health flows.** Health dashboard (next vet visit,
  ...
  _Accept:_ the dashboard is genuinely useful at a glance; the emergency flow
...
Scout with health questions (including ones it should _refuse_ to answer like a
vet would).
...
_Goal: diet, grooming, exercise, enrichment, socialization, potty, leash, and humane‑boundaries — each with data, UI, and Scout tools — tied together on the Today home._
...
- **M4.1 — Diet & nutrition.** `diet_plan` + `meal_log` schema/migrations; food
  ...
  / `log_meal` tools.
  _Accept:_ the default dog gets a sane portion recommendation that changes
...
- **M4.2 — Grooming, coat, nails, ears, dental.** `grooming_log` schema/migration;
  ...
  _Accept:_ the short‑double‑coat default gets a light realistic schedule; a
...
- **M4.3 — Exercise & activities + dog sports.** `activity_log` schema/migration;
  ...
  _Accept:_ the working‑breed default shows an honest (high) daily target and a
  nosework on‑ramp module available; logging + summaries work; a hot‑day warning
  shows; Scout suggests breed‑appropriate outlets.
- **M4.3a — Protection / Bite‑Sport (IGP) track** _(opt‑in)_. The fourth program
  ...
  checks) **and** a hard gate where the owner affirms _"I'm training under a
  certified decoy / IGP‑Schutzhund club,"_ the sport‑bite modules
  ...
  _Accept:_ the track is off by default; opting in shows the warning + foundation
  ...
  the framing. _(Cuttable if you'd rather not ship a protection track at all.)_
- **M4.4 — Enrichment & toys.** `toy` schema/migration; toy inventory by category
  ...
  _Accept:_ manage a toy inventory; get a rotation schedule; Scout recommends
...
- **M4.5 — Socialization tracker.** `socialization_item` schema/migration; the
  **critical‑window checklist** (people/dogs/animals/environments/surfaces/
  ...
  _Accept:_ a puppy intake surfaces an urgent, well‑structured socialization plan;
...
- **M4.6 — Potty / house training.** `potty_log` schema/migration; schedule by
  ...
  _Accept:_ a puppy gets a realistic potty schedule + module; logging works;
...
- **M4.7 — Leash manners.** Loose‑leash protocol as a program module; equipment
  ...
  past a threshold.
  _Accept:_ the loose‑leash module is concrete and humane; the equipment screen
...
- **M4.8 — Boundaries & humane correction.** The "healthy discipline" module
  ...
  loss of access, consistency; the _what‑not‑to‑do and why_; resource‑guarding &
  ...
  _Accept:_ the module reads as expert, humane, and _practical_ (not preachy);
...
- **M4.9 — The Today home, for real.** Pull every time‑bound thing into one
  ...
  _Accept:_ the Today screen is the screen you'd actually open every morning;
...
_Goal: make it feel smart, proactive, and grounded._

- **M5.1 — Multimodal Scout.** Image input to chat; `analyze_photo` use cases —
  body‑condition‑score _assist_, coat/skin/dental _observations_ (not
  ...
  _Accept:_ upload a photo of a dog, get a careful BCS assist with "confirm with
...
- **M5.2 — Proactive assistant.** A weekly check‑in / daily‑plan card Scout
  ...
  _Accept:_ the weekly card is genuinely useful and well‑timed; opting out
...
- **M5.3 — Grounded knowledge.** Expand the breed library and the `knowledge/`
  ...
  _Accept:_ on factual questions Scout pulls from the curated set; the breed
...
- **M5.4 — Milestones, streaks, sharing.** `milestone` polish; phase‑graduation
  ...
  _Accept:_ milestones populate automatically + manually; a graduation feels
...
- **M5.5 — Voice mode (optional).** On‑device STT/TTS in the mobile app for
  ...
  _Accept:_ a training session can be run hands‑free on a real phone; degrades
  gracefully where unsupported. _(Cuttable if you don't want it.)_
- **M5.6 — Multi‑dog & family sharing.** `dog_member` join table + roles;
  ...
  _Accept:_ two accounts share one dog, both log, both get the right reminders,
  permissions hold.
...
_Goal: ship v1.0.0 — secure, reliable, observable, in the app stores and on the web._
...
- **M6.1 — Security pass.** Authz audit (every route + tool re‑checked for
  ...
  _Accept:_ findings fixed or ticketed‑with‑rationale; no high/critical open.
- **M6.2 — Reliability.** Error tracking (Sentry, free tier 🔒), structured logs,
  ...
  _Accept:_ kill Postgres / kill Anthropic / blow a token budget → app degrades
...
- **M6.3 — Performance.** Query/index review, pagination everywhere, image
  ...
  _Accept:_ p95 targets met on key screens & the chat endpoint under the load
...
- **M6.4 — Test depth.** Unit + integration coverage to an agreed bar; web E2E
  ...
  _Accept:_ the critical journeys are E2E‑covered and green in CI; coverage bar
  met.
- **M6.5 — AI quality & safety eval harness.** A suite of golden conversations +
  ...
  _Accept:_ persona/knowledge changes can't merge if a safety probe regresses;
  the golden set documents expected behavior.
- **M6.6 — Deployment ⚙️🔒.** Pick the path (self‑host Docker + Caddy + Tailscale,
  _or_ cloud: API on Fly.io/Render + managed Postgres + R2/S3); production
  `infra/`; staging env; CD pipeline (tag → build → deploy); runbook.
  _Accept:_ a tagged commit deploys to staging then production with one approval;
...
- **M6.7 — Mobile & web release 🔒.** App icon/splash, store assets & listings,
  ...
  🔒.
  _Accept:_ the app is in TestFlight/Play review (or live), the web app is live,
...
- **M6.8 — Docs & legal 🔒.** In‑app help/onboarding polish, a short user guide,
  **Privacy Policy** + **Terms of Service** + the medical/training disclaimer
  ...
  _Accept:_ legal docs linked in‑app and at sign‑up; help content covers the FAQ;
  a support address works.
- **M6.9 — v1.0.0 🧪.** Final UI/UX pass with you, changelog, tag `v1.0.0`,
  launch.
  _Accept:_ you've signed off; it's tagged, deployed, monitored, supported.
...
- **The "healthy punishment" tension** → I've reframed it humane‑first and
  ...
  _before_ Phase 1 ships, because it threads through the persona, the curriculum
